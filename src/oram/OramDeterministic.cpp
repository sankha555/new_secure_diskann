    ...
    for (int i = 0; i < num_levels; i++) {
        ...
        for (Block b: blocks) {
            if (b.index != blockIndex) {
                ...
                write_back.addBlock(Block(block_size));
...
int* OramDeterministic::batch_access(Operation op, int blockIndex, int *newdata) {
    int *data = new int[block_size];
    int oldLeaf = position_map[blockIndex];
    position_map[blockIndex] = rand_gen->getRandomLeafWithBound(num_leaves);
    Block *targetBlock = NULL;
    
    {
        ...
        // Batch Write
        for (int i = 0; i < num_levels; i++) {
            vector<Block> blocks = bkts[i].getBlocks();
            Bucket write_back = Bucket();
            for (Block b: blocks) {
                if (b.index != blockIndex) {
                    write_back.addBlock(Block(b));
